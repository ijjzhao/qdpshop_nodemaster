...
module.exports = class extends Base {
  ...
  async submitAction() {
    ...
    // TODO：如果是微信的卡券 还要核销 ? 下单了没有付钱 卡券怎么办
    let card = await this.model('coupon_user').where({ coupon_id: couponId }).find();
    if (card.code) {
      let service = this.service('wxcard', 'api');
      if (await service.checkCode(card.code) && await service.consumeCode(card.code)) {
        console.log('微信卡券正常核销')
      } else {
        return this.fail('卡券状态异常');
      }
    }
